...
const Players = (() => {
    ...
    function setPlayers(e) {
        ...
        if (options[0] === e.target.value) {
            playersArray.push(playerFactory(playerNameValues[0].value !== '' ? playerNameValues[0].value : 'Player 1', options[0]));
            playersArray.push(playerFactory(playerNameValues[1].value !== '' ? playerNameValues[1].value : 'Player 2', options[1]));
        } else {
            playersArray.push(playerFactory(playerNameValues[0].value !== '' ? playerNameValues[0].value : 'Player 1', options[1]));
            playersArray.push(playerFactory(playerNameValues[1].value !== '' ? playerNameValues[1].value : 'Player 2', options[0]));
        }
...
const Game = (() => {
    ...
    function markItem(e) {
        if (gameBoard.determineWinner(currentPlayerMark) === true) {
            ...
            const dataMarkValue = e.target.getAttribute('data-mark');
            const dataBoxNumber = parseInt(e.target.getAttribute('data-box'));
            ...
            if (gameBoard.determineWinner(currentPlayerMark) === true) {
                if (Players.playersArrayMark(0) === currentPlayerMark) {
                    gameBoard.displayManagement('resultsText', `${Players.playersArrayName(0)} playing as ${currentPlayerMark} won`);
                } else {
                    gameBoard.displayManagement('resultsText', `${Players.playersArrayName(1)} playing as ${currentPlayerMark} won`);
                }
